...
function TopBarAffinity() {
  const history = useHistory();
  const location = history.location;
  const windowType = location.pathname.split("/")[2];
  return (
    <div className="inline-flex justify-end space-x-4">
      <button
        className={
          windowType === "Propuestos"
            ? "px-8 py-2 bg-unancor-blue font-sans text-sm font-bold text-white"
            : "px-8 py-2 bg-gray-100 font-sans text-sm font-bold"
        }
        onClick={() => history.push("/HomeSerps/Propuestos")}
      >
        Propuesta
      </button>
      {windowType !== "Propuestos" && (
        <button
          className={
            windowType === "Competencias"
              ? "p-2 px-10 text-sm cursor-pointer bg-unancor-blue font-bold text-white"
              : "p-2 px-10 text-sm cursor-pointer bg-gray-100 font-bold"
          }
          onClick={() => history.push("/HomeSerps/Competencias")}
        >
          Competencia
        </button>
      )}
      <button
        className={
          windowType === "CrearAnalisisPasoUno"
            ? "inline-flex px-8 py-2 bg-unancor-blue text-white text-center font-sans text-sm font-bold"
            : "inline-flex p-2 bg-gray-100 opacity-30 font-sans text-sm font-bold"
        }
        onClick={() => windowType === "Propuestos" ? history.push("/HomeSerps/Competencias") : history.push("/HomeSerps/CrearAnalisisPasoUno")}
      >
        {windowType === "CrearAnalisisPasoUno" ? "SERPS" : "Añadir componente"}
        {windowType !== "CrearAnalisisPasoUno" && (
          <svg
            className="mt-1 ml-2"
            width="14"
