...
function Sidebar() {
  ...
  const onClickSelectedMenu = (onClickMenuNumber) => {
    setSelectedMenu(onClickMenuNumber);
  };

  return (
    <div className="bg-unancor-blue w-20 flex flex-col h-full absolute z-10 mt-10">
      <div className="mt-10 w-full">
        {ItemsSidebar.map(({ title, Icon }, index) => (
          <div
            className={
              selectedMenu === index
                ? "cursor-pointer text-center mb-6 bg-gray-200"
                : "cursor-pointer text-center mb-6"
            }
            key={index}
            onClick={() => onClickSelectedMenu(index)}
          >
            <Icon selectedMenu={selectedMenu} />
            <p
              className={
                selectedMenu === index
